...
def process_one_path(path: str) -> pd.DataFrame:
    ...
    trivial_para_neighborhood = {dominant_para_seq: 0}
    para_neighborhood = trivial_para_neighborhood
    ...
    for key, value in meta.items():
        para_phenotypes_df[key] = value

    for key, value in dominant_para_taxon.items():
        para_phenotypes_df[f"Reference Taxon {key}"] = value

    return para_phenotypes_df
...
master_df.to_csv("${BATCH_PATH}/dominant-para-phenotypes.csv")
print("${BATCH_PATH}/dominant-para-phenotypes.csv saved")
EOF
