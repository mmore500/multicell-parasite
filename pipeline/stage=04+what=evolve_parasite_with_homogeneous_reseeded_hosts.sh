...
CONTINUATION_TEMPLATE="$(
  cat stage=${STAGE}+what=evolve_parasite_with_homogeneous_reseed/evolve_parasite_with_homogeneous_reseeded_hosts.slurm.sh.jinja | sed 's/^/  /'
)"
...
for host_prototype in ${HOST_PROTOTYPE_GLOB}; do
  ...
  j2 --format=yaml -o "${SBATCH_SCRIPT_PATH}" "stage=${STAGE}+what=evolve_parasite_with_homogeneous_reseeded_hosts/evolve_parasite_with_homogeneous_reseeded_hosts.slurm.sh.jinja" << J2_HEREDOC_EOF
attempt: ${ATTEMPT}
config_and_run: |
${CONFIG_AND_RUN}
continuation_template: |
