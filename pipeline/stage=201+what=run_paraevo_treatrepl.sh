...
TREATMENTS="${2:-"evohost-polyhost evohost-monohost ecohost-polyhost evohost-monohost"}"
...
CONTINUATION_TEMPLATE="$(
  cat stage=${STAGE}+what=run_paraevo_treatrepl/run_paraevo_treatrepl.slurm.sh.jinja | sed 's/^/  /'
)"
...
for replicate in ${REPLICATES}; do
  ...
  j2 --format=yaml -o "${SBATCH_SCRIPT_PATH}" "stage=${STAGE}+what=run_paraevo_treatrepl/run_paraevo_treatrepl.slurm.sh.jinja" << J2_HEREDOC_EOF
batch: ${BATCH}
config_and_run: |
${CONFIG_AND_RUN}
continuation_template: |
